...
export class CirclesService {
  ...
  async generateInviteCode(circleId: number, userId: number) {
    ...
    if (
      ...
      throw new ForbiddenException(
        'Only owners and moderators can generate invite codes',
      );
    }
  ...
  async removeMember(
    ...
    const requestingMember = circle.members.find(
    ...
    if (
      requestingMember.role !== MemberRole.OWNER &&
      requestingMember.role !== MemberRole.MODERATOR
    ) {
      throw new ForbiddenException(
        'Only owners and moderators can remove members',
      );
    }
    ...
    if (
      requestingMember.role === MemberRole.MODERATOR &&
      targetMember.role === MemberRole.MODERATOR
    ) {
